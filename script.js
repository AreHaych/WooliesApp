...
[
    // [0, 'test for 0'], 
    [1, 'Bread', 'images/bread.jpg']
    //  [1, 'Cereal'], [1,'Cakes?'], [1, 'Crumpets'], [1, 'Health care (vitamins, foods)'], [1, 'Honey'], [1, 'Nutella'], [1, 'High Quality Peanutbutter'],[1, 'Specialty Jams'], [1, 'Muffins'], [1, 'Muesli and oats'], 
 ...
 * @param {array} 
 */
function makeItemObject([aisle, name, image]) {
...
function getRandomNumber() {
    return Math.floor(Math.random() * itemObjectArray.length); //issue   
}
...
let currentItemMessageEle = document.getElementById('current-item-message').innerHTML = `What aisle is ${currentItem.itemName} located in?`;
let userInput = document.getElementById('user-input');
let userFeedbackEle = document.getElementById('user-feedback');
let currentItemImage = document.getElementById('current-item-image').src = currentItem.itemImage;


//event listener for button when user presses enter
let textArea = document.getElementById('user-input').addEventListener('keyup', function(event) {
    if(event.key === 'Enter') {
        userInputCheck();
    }
});
...
function userInputCheck() {
    if(userInput.value) {
        if(userInput.value == currentItem.itemAisle) {
            ...
            userFeedbackEle.style = 'color: green';
...
function resetValues() {
    ...
    currentItemMessageEle = document.getElementById('current-item-message').innerHTML = `What aisle is ${currentItem.itemName} located in?`;
